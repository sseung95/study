...
console.log(total / oddsArr.length);

// 3
for (const [key, value] of oddsArr) {
...
const convertCamelCase = function () {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // const word = first + second.slice(0, 1).toUpperCase() + second.slice(1);
    const word = `${first}${second.replace(
      second[0],
      second[0].toUpperCase()
    )}`;
    const finalWord = `${word.padEnd(20) + '✅'.repeat(i + 1)}`;
...
for (const flight of flightArr) {
  ...
  const result = `${type.startsWith('_Delayed') ? '🔴' : ''}${type.replaceAll(
    '_',
    ' '
